...
class FireStoreViewModel {
    ...
    func addPartnerPokemon(email: String, partnerPokeNumber: Int) {
            // FirebaseService를 통해 데이터 추가
        FireStore.addPartnerPokemon(email: email, partnerPokeNumber: partnerPokeNumber)
    }
    ...
    //파트너 포켓몬 가져오기위한 로직
    func getPartnerData(for email: String) {
        FireStore.getPartnerData(for: email)
                .sink { completion in
                    // 데이터를 가져오는 데에 실패한 경우 처리
                    switch completion {
                        case .failure(let error):
                ...
                } receiveValue: { partnerPokemon in
                    // 가져온 데이터를 처리
